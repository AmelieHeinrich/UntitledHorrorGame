...
Texture::Texture(TextureType type, TextureLayout layout, int width, int height, DXGI_FORMAT format)
    : _Width(width), _Height(height), _Type(type), _Format(format)
{
...
void Texture::MakeUnorederedAccess()
{
...
Ref<Texture> Texture::CreateFromImage(const Image& image)
{
    ...
    Ref<Texture> gpu_resident = CreateRef<Texture>(TextureType::TwoDimension, TextureLayout::ShaderResource, image.Width, image.Height, DXGI_FORMAT_R8G8B8A8_UNORM);
