...
void RenderContext::UnbindRenderTarget()
{
    ...
    _Data._DeviceContext->OMSetRenderTargets(1, rtv, nullptr);
}
...
void RenderContext::UnbindGraphicsVertexConstantBuffer(i32 slot)
{
    ID3D11Buffer* nullBuffer[1] = { nullptr };
    _Data._DeviceContext->VSSetConstantBuffers(slot, 1, nullBuffer);
}

void RenderContext::UnbindGraphicsPixelConstantBuffer(i32 slot)
{
    ID3D11Buffer* nullBuffer[1] = { nullptr };
    _Data._DeviceContext->PSSetConstantBuffers(slot, 1, nullBuffer);
}

void RenderContext::UnbindGraphicsShaderResource(i32 slot)
{
    ID3D11ShaderResourceView* nullBuffer[1] = { nullptr };
    _Data._DeviceContext->PSSetShaderResources(slot, 1, nullBuffer);
}

void RenderContext::UnbindGraphicsSampler(i32 slot)
{
    ID3D11SamplerState* nullBuffer[1] = { nullptr };
    _Data._DeviceContext->PSSetSamplers(slot, 1, nullBuffer);
}

void RenderContext::BindComputePipeline(Ref<ComputePipeline> pipeline)
{
    _Data._DeviceContext->CSSetShader(pipeline->_Shader, nullptr, 0);
}

void RenderContext::BindComputeConstantBuffer(Ref<Buffer> buffer, i32 slot)
{
    _Data._DeviceContext->CSSetConstantBuffers(slot, 1, &buffer->_Buffer);
}

void RenderContext::BindComputeUnorderedViewTexture(Ref<Texture> texture, i32 slot)
{
    _Data._DeviceContext->CSSetUnorderedAccessViews(slot, 1, &texture->_UAV, nullptr);
}

void RenderContext::BindComputeUnorderedViewBuffer(Ref<Buffer> texture, i32 slot)
{
    _Data._DeviceContext->CSSetUnorderedAccessViews(slot, 1, &texture->_UAV, nullptr);
}

void RenderContext::BindComputeShaderResource(Ref<Texture> texture, i32 slot)
{
    _Data._DeviceContext->CSSetShaderResources(slot, 1, &texture->_SRV);
}

void RenderContext::BindComputeSampler(Ref<Sampler> sampler, i32 slot)
{
    _Data._DeviceContext->CSSetSamplers(slot, 1, &sampler->_Sampler);
}

void RenderContext::UnbindComputeConstantBuffer(i32 slot)
{
    ID3D11Buffer* nullBuffer[1] = { nullptr };
    _Data._DeviceContext->CSSetConstantBuffers(slot, 1, nullBuffer);
}

void RenderContext::UnbindComputeUnorderedViewTexture(i32 slot)
{
    ID3D11UnorderedAccessView* nullBuffer[1] = { nullptr };
    _Data._DeviceContext->CSSetUnorderedAccessViews(slot, 1, nullBuffer, nullptr);
}

void RenderContext::UnbindComputeUnorderedViewBuffer(i32 slot)
{
    ID3D11UnorderedAccessView* nullBuffer[1] = { nullptr };
    _Data._DeviceContext->CSSetUnorderedAccessViews(slot, 1, nullBuffer, nullptr);
}

void RenderContext::UnbindComputeShaderResource(i32 slot)
{
    ID3D11ShaderResourceView* nullBuffer[1] = { nullptr };
    _Data._DeviceContext->CSSetShaderResources(slot, 1, nullBuffer);
}

void RenderContext::UnbindComputeSampler(i32 slot)
{
    ID3D11SamplerState* nullBuffer[1] = { nullptr };
    _Data._DeviceContext->CSSetSamplers(slot, 1, nullBuffer);
}
...
void RenderContext::Dispatch(u32 X, u32 Y, u32 Z)
{
    _Data._DeviceContext->Dispatch(X, Y, Z);
}
